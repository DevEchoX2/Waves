document.addEventListener("DOMContentLoaded", function () {
  ...
  window.showToast = function (type, message, iconName) {
    ...
    const icons = {
      ...
      error: iconName || "x-circle",
      info: iconName || "info",
    };
    const iconNameResolved = icons[type] || iconName || "info";

    const content = document.createElement("div");
    content.className = "toast-content";
    content.innerHTML = `<i data-lucide="${iconNameResolved}" class="toast-icon"></i><span>${message}</span>`;
